...
public class U4E94 implements SpecificExtractor {
    ...
    public Strokes extractStrokes(Contours contours, Points points) {
        ...
        try {
            ...
            tempList3.add(points.get(Points.INTER).get(0));
            tempList3.add(points.get(Points.INTER).get(1));
            tempList3.sort();
            PointList tempList4 = new PointList();
            tempList4.add(points.get(Points.INTER).get(-1));
            tempList4.add(points.get(Points.INTER).get(-2));
            tempList4.sort();
            //右上到左下
            strokes.get(2).addList(contours.getMatchContour(
                    tempList3.get(1), points.get(Points.INTER).get(2)), true);
            strokes.get(2).addList(contours.getMatchContour(
                    points.get(Points.INTER).get(-3), tempList4.get(0)));
            //左上到右下
            strokes.get(3).addList(contours.getMatchContour(
                    tempList3.get(0), points.get(Points.INTER).get(2)), true);
            strokes.get(3).addList(contours.getMatchContour(
                    points.get(Points.INTER).get(-3), tempList4.get(1)));
